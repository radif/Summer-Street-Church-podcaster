...
-(id)initWithGenreList:(NSMutableDictionary *)Dictionary
{
    ...
    {
        NSLog(@"Failed to open resource dictionary, can not find file:%s",[dataPath cStringUsingEncoding:NSUTF8StringEncoding]);
        [self autorelease];
        return NULL;
    }
...
-(BOOL)examineFile:(NSString *)Path
{
    ...
    BOOL rtn = [mp3Header openFile:path withTag:position];
...
-(BOOL)examineFile:(NSString *)Path hasValidMp3Header:(BOOL)validMp3
{
	[self releaseAttributes];
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    path = [Path copy];
